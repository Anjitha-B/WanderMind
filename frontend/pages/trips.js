...
export default function MyTrips() {
    ...
    const [selectedTrip, setSelectedTrip] = useState(null); // For viewing full details
    ...
    const fetchUserTrips = async () => {
        try {
            // 1. Fetch current authenticated user metadata context
            ...
            // 2. Call your secure FastAPI pipeline gateway
            const response = await fetch(`http://localhost:8000/get-trips/${user.id}`);
            if (!response.ok) throw new Error("Failed to pull dashboard historical rows");

            const data = await response.json();
            setTrips(data.trips);
        } catch (error) {
            console.error("Dashboard synchronization sync error:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-10 px-6 sm:px-10">
            <div className="max-w-6xl mx-auto">
                {/* Header Context Action Layout Row */}
                        ...
                        <div className="lg:col-span-1 space-y-4 overflow-y-auto max-h-[75vh] pr-2">
                            {trips.map((trip) => (
                                ...
                                >
                                    ...
                                    {/* Sub-interests tags element wrapper mapping strings */}
